...
/*
    ...
    To create separate error messages for QUARC and RCP, create a second comment on the
    same line for RCP. When the error messages are generated for RCP, this second comment
...
enum tag_error
{
    ...
    QERR_BUFFER_OVERFLOW,                                               /* For a read operation, the buffer has overflowed. For a write operation, there is no more data left in the buffer. The sampling frequency is too fast for the rate at which data is being read from or written to the buffer. For HIL Timebase blocks or HIL tasks, oversampling has occurred. */
    ...
    QERR_INVALID_PEER_OPTION,                                           /* Invalid "peer" option was specified. Valid values for the "peer" option are "one", "any" and "broadcast". */
    ...
    QERR_PGR_INVALID_CUSTOM_IMAGE_SIZE,                                 /* Starting the PGR camera failed because the custom image size is invalid. */
    QERR_PGR_INVALID_IMAGE_DIMS,                                        /* The dimensions of the PGR image are not valid. The image must have 3 dimensions. */
    ...
    QERR_NO_QCC_FRAMEWORK_LICENSE,                                      /* You do not have a valid license for the Quanser Cooperative Control (QCC) Framework. To configure licensing use the Configure License utility found under Start Menu/All Programs/Quanser for your product. */
    ...
    QERR_HIL_READ_ANALOG_BUFFER_NOT_SUPPORTED,                          /* The hil_read_analog_buffer function and HIL Read Analog Buffer block not supported by this particular card. */
    ...
    QERR_HIL_TASK_CREATE_OTHER_READER_OTHER_WRITER_NOT_SUPPORTED,       /* The hil_task_create_other_reader_other_writer function and HIL Read Other Write Other Timebase block not supported by this particular card. */
    ...
    QERR_VISION_CAMERA_NOT_FOUND,                                       /* The camera is not valid for image capture. */
    ...
    QERR_NO_INTERNAL_USE_LICENSE,                                       /* The "Active during normal simulation" feature is only licensed for internal Quanser use. Due to safety and liability concerns, this feature is not enabled, and will not be enabled, for client installations. */
    ...
    QERR_OPTITRACK_INVALID_CALIBRATION_FILE,                            /* Unable to load OptiTrack calibration file. Make sure the file path is correct. */
    ...
    QERR_MISMATCHED_CLOCK_FREQUENCY,                                    /* One of the clock frequencies specified does not match the frequency of the other channels. This card only allows one clock frequency to be set for all PWM channels. */
    ...
    QERR_NO_QBALL_X4_LICENSE,                                           /* You do not have a valid license for the QBall-X4 vehicle. To configure licensing use the Configure License utility found under Start Menu/All Programs/Quanser for your product. */
    ...
    QERR_INVALID_HOST_PACKET,                                           /* A Host block appears to be incompatible with the current version of QUARC because it has sent invalid data. It may be necessary to rebuild the model. */ /* A Host block appears to be incompatible with the current version of RCP because it has sent invalid data. It may be necessary to recompile the VI. */
    ...
    QERR_NO_KINOVA_LICENSE,                                             /* You do not have a valid license for the Kinova robot blockset you are using. To configure licensing use the Configure License utility found under Start Menu/All Programs/Quanser for your product. */
    ...
    QERR_INVALID_BOARD_VERSION,                                         /* Version information retrieved from the HIL board does not match the driver. It may be necessary to update QUARC to get the latest drivers. */
